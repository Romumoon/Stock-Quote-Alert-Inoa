...
using IHost host = Host.CreateDefaultBuilder(args)
    .ConfigureServices((context, services) => {
        ...
        services.AddTransient<IMonitorService, StockMonitorService>();
    })
...
var monitorService = host.Services.GetRequiredService<IMonitorService>();
...
await monitorService.ExecuteAsync(ticker, sellPrice, buyPrice);

static bool ValidateArgsCount(string[] args, ILogger logger)
{
...
static bool TryParseParameters(string[] args, out string ticker, out decimal sell, out decimal buy, ILogger logger)
{
    ticker = string.Empty;
    sell = 0;
    buy = 0;
...
static bool ValidateConfiguration(IConfiguration config, ILogger logger)
{
